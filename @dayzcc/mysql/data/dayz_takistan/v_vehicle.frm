TYPE=VIEW
query=select `iv`.`id` AS `instance_vehicle_id`,`v`.`id` AS `vehicle_id`,`iv`.`instance_id` AS `instance_id`,`i`.`world_id` AS `world_id`,`v`.`class_name` AS `class_name`,`iv`.`worldspace` AS `worldspace`,`iv`.`inventory` AS `inventory`,`iv`.`parts` AS `parts`,`iv`.`damage` AS `damage`,`iv`.`fuel` AS `fuel` from (((`dayz_takistan`.`instance_vehicle` `iv` join `dayz_takistan`.`world_vehicle` `wv` on((`iv`.`world_vehicle_id` = `wv`.`id`))) join `dayz_takistan`.`vehicle` `v` on((`wv`.`vehicle_id` = `v`.`id`))) join `dayz_takistan`.`instance` `i` on((`iv`.`instance_id` = `i`.`id`)))
md5=397383e2a8ddb09645563ac136a97fff
updatable=1
algorithm=0
definer_user=root
