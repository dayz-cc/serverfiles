...
// GHK custom crafting menu system just use Grave object to test for now  //GHK Also add burn zone fire check-No removed them
   // if (((inflamed cursorTarget || cursorTarget getVariable ["addfire",false]) && cursorTarget distance player < 10) && _canDo && dz_use_crafting && !crafting_menu_open) then {
   if (((inflamed cursorTarget) && cursorTarget distance player < 10) && _canDo && dz_use_crafting && !crafting_menu_open) then {
...
//GHK Booby Trap actions.
if (!isNull cursorTarget and !_inVehicle and (player distance cursorTarget < 8)) then {	//Has some kind of target..for booby trap deactivation that has range of 5
	_isGrave = cursorTarget isKindOf "Grave"; 
	_GraveTrigger = cursorTarget getVariable["traptrigger",player]; //just return 'player' if no variable found
	if (_isGrave&&(_GraveTrigger isKindOf "EmptyDetector")) then {
		     
			if (s_player_removeBomb < 0) then {
				s_player_removeBomb = player addAction [("<t color=""#c30000"">" + ("Remove Exploding Booby Trap") +"</t>"),"custom\removeBomb.sqf",cursorTarget,6,false,true,"",""];
			};
		
	} else 
	{ 
		player removeAction s_player_removeBomb;
...
if (!isNull cursorTarget and !_inVehicle and (player distance cursorTarget < 4)) then {	//Has some kind of target
	...
	_isKennel = cursorTarget isKindOf "Land_psi_bouda";
 
	if (_isKennel) then {
        if (s_player_doggy < 0) then {
            s_player_doggy = player addaction["Call for Dog", "custom\tame_dog.sqf", cursorTarget, 1, false, true, "",""];
        };
    } else {
	...
	*/
	...
	//hint format ["_hasFuelE = %1, _isFuel =%2,  _canDo = %3, s_player_fillfuel = %4" ,_hasFuelE,_isFuel,_canDo,s_player_fillfuel];
	...
	//if ((inflamed cursorTarget || cursorTarget getVariable ["addfire",false]) and _hasRawMeat and _canDo) then {
	...
	//if ((inflamed cursorTarget || cursorTarget getVariable ["addfire",false]) and (_hasbottleitem and _hastinitem) and _canDo) then {
	...
	//GHK Creating/Removing stashes with etool from fireplaces that have already been placed by the player. can there be legitimate char ids of "0"?
	if(!(inflamed cursorTarget) and cursorTarget isKindOf "Land_Fire_DZ" and _canDo and _hasEtool and _ownerID =="0") then {
		if ((s_player_firestash < 0) and !(inflamed cursorTarget) and (player distance cursorTarget < 3)) then {
			s_player_firestash = player addAction ["Create a Stash Point", "custom\player_makestash.sqf",cursorTarget, 0, false, true, "",""];
		};
	} else {
	...
	//if another player has an etool, let them remove the stash as well
	if((!inflamed cursorTarget) and cursorTarget isKindOf "Land_Fire_DZ" and _canDo and _hasEtool and _ownerID !="0") then {
		if ((s_player_firestashout < 0) and !(inflamed cursorTarget) and (player distance cursorTarget < 3)) then {
			s_player_firestashout = player addAction ["Remove the Stash", "custom\player_removeStash.sqf",cursorTarget, 0, false, true, "",""];
		};
	} else {
...
if(!dayz_cannot_have_dog) then {
//Dog actions on player self
_dogHandle = player getVariable ["dogID", 0];
if (_dogHandle > 0) then {
	// GHK: _dog = _dogHandle getFSMVariable "_dog";
	_ownerID = "0";
	if (!isNull cursorTarget) then { _ownerID = cursorTarget getVariable ["characterID","0"]; };
	if (_canDo and !_inVehicle and _ownerID != dayz_characterID) then { //removed ' and alive _dog ' check
	
	_nearbydogs = (nearestObjects [player, ['Pastor','Fin'], 40]);
	//_charID = format["%1",dayz_characterID];
	//_playerdognearby = ({(_x getVariable["CharacterID","0"]) == player getVariable ["characterID", "0"])} count _nearbydogs); 
	playerdognearby= 0;
	
		{
		
			if(_x getVariable ["CharacterID","0"] == player getVariable ["characterID", "0"]) exitWith {playerdognearby= 1;};
		...
		*/
		...
		//GHK Hack in the new commands instead of default
		if (s_player_movedog < 0) then {
			s_player_movedog = player addAction ["Dog |<t color='#00a6eb'> Stay here</t>","addin\dogCommand.sqf",["stay"],-10,false,true,"","playerdognearby > 0"];
		};
		if (s_player_speeddog < 0) then {
			s_player_speeddog = player addAction ["Dog |<t color='#00a6eb'> Follow me</t>","addin\dogCommand.sqf",["follow"],-11,false,true,"","playerdognearby > 0"];
		};
		if (s_player_calldog < 0) then {
			s_player_calldog = player addAction ["Dog |<t color='#00a6eb'> Find animal</t>","addin\dogCommand.sqf",["find"],-12,false,true,"","playerdognearby > 0"];
		};
		
		if (s_player_movedog2 < 0) then {
			s_player_movedog2 = player addAction ["Dog |<t color='#00a6eb'> Whistle For Dog</t>","addin\dogCommand.sqf",["whistle"],-9,false,true,"","('CDF_dogtags' in Weapons player)"];
		};
		...
		if(_hasRawMeat) then {
			if (s_player_feeddog2 < 0) then {
			
				s_player_feeddog2 = player addAction ["Dog |<t color='#00a6eb'> Feed Dog</t>","addin\dogCommand.sqf",["feed"],-9,false,true,"","_playerdognearby > 0"];
			};				
		}else{
			player removeAction s_player_feeddog2;	
			s_player_feeddog2 =	-1;
		};	
		//
		player removeAction s_player_movedog2;	
		s_player_move_dog2 =	-1;
    ...
    if (s_player_makeBomb < 0) then {
        s_player_makeBomb = player addAction [("<t color=""#c30000"">" + ("Place Exploding Booby Trap") +"</t>"),"custom\setAlarm.sqf","",5,false,true,"",""];
    };
} else {
  ...
  if ("ItemEtool" in weapons player) then {
        hasBombItem = true;
    } else {
        hasBombItem = false;
    };
    ...
    if ((hasBomb) && ("HandGrenade_West" in _mags)) then {
        if (s_player_makeBomb < 0) then {
            s_player_makeBomb = player addAction [("<t color=""#c30000"">" + ("Place Exploding Booby Trap") +"</t>"),"custom\setBomb.sqf","",5,false,true,"",""];
        };
    } else {
        player removeAction s_player_makeBomb;
        s_player_makeBomb = -1;
