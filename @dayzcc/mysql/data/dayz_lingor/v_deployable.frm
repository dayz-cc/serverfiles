TYPE=VIEW
query=select `id`.`id` AS `instance_deployable_id`,`d`.`id` AS `vehicle_id`,`d`.`class_name` AS `class_name`,`s`.`id` AS `owner_id`,`p`.`name` AS `owner_name`,`p`.`unique_id` AS `owner_unique_id`,`s`.`is_dead` AS `is_owner_dead`,`id`.`worldspace` AS `worldspace`,`id`.`inventory` AS `inventory` from (((`dayz_lingor`.`instance_deployable` `id` join `dayz_lingor`.`deployable` `d` on((`id`.`deployable_id` = `d`.`id`))) join `dayz_lingor`.`survivor` `s` on((`id`.`owner_id` = `s`.`id`))) join `dayz_lingor`.`profile` `p` on((`s`.`unique_id` = `p`.`unique_id`)))
md5=8b623e56e41553bd0d3ec5a5e8575303
updatable=1
algorithm=0
definer_user=root
