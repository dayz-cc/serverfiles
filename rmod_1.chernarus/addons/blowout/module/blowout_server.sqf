...
bl_damage = {
	private["_isinbuilding","_emp","_bloodlevel","_healtharray"];
	_emp = _this select 0;
		...
		if (!isNull _jednotka) then {
			...
			if (!(_jednotka hasWeapon ns_blow_itemapsi)) then {
				...
				if (!_isinbuilding) then {
					...
					_healtharray = _jednotka getVariable ["unithealth",[]];
					if (count _healtharray > 0) then {
						_unithealth = _jednotka getVariable "unithealth"; 
						_bloodlevel = (_unithealth select 0);
						_bloodlevel = (dz_ai_blowout_bloodloss_fraction)*_bloodlevel;
						_jednotka setVariable ["unithealth",[(_bloodlevel),0,false]];
						
						diag_log ("found DZAI health unit. New blood is:");
						diag_log(_bloodlevel);
					};					
...
private["_emp_tg_namalsk","_temp_dzn_ns_bloodsucker_den"];
...
while {true} do {
 ...
 if (isNil("ns_blow_prep")) then { ns_blow_prep = false; };
  ...
  //GHK, lets max out bloodsucker spawn chance during blowout for the bloodsucker zone.
 // dzn_ns_bloodsucker_den=400;
  //publicVariable "dzn_ns_bloodsucker_den";
  ...
  if (ns_blow_status) then {
    ...
    sleep 4;
    _bul = [] call bl_flashes;
   // GHK, lets call for server side DZAI Health system
   bul = [_emp_tg_namalsk] call bl_damage;
  ...
  diag_log format["[NAC BLOWOUT SERVER] :: Blowout finished (ns_blow_status = %1)", ns_blow_status];
   //GHK, lets change bloodsucker spawn chance after blowout for the bloodsucker zone.
   //1/400 to 10/400 chance, 0.25 to 2.5 % chance..no 0.25 to 10% chance 
 dzn_ns_bloodsucker_den =  floor(random 30) + 10; //floor(random 39) + 1; //_temp_dzn_ns_bloodsucker_den;
 publicVariable "dzn_ns_bloodsucker_den";
  diag_log format["SERVER: GHK dzn_ns_bloodsucker_den is: " + str( dzn_ns_bloodsucker_den ) ];
};