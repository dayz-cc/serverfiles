...
//diag_log format["CLIENT: GHK buildings dz_bloodsucker_count is: " + str(dz_bloodsucker_count) + " dz_max_globalbloodsucker is : " + str(dz_max_globalbloodsuckers) + "building is in zone: " +str((_obj getVariable["spawn_ns_bloodsucker",0]>0))];
	 private["_meters"];
 ...
 // diag_log ("ghk spawn bloodsucker called");
	...
	if(_obj getVariable["spawn_ns_bloodsucker",0]>0) then { 
		...
		if(dayz_BSZonePos <= 2800 ) then { //check if the player is within the  bloodsucker zone 
			//lets max out the local bloodsuckers in area to  dz_max_bloodsuckers
			if(({alive _x} count (_originalPos nearEntities ["ns_bloodsucker", 560])) <  dz_max_bloodsuckers) then {
				...
				//diag_log format ["building of type %1 spawned bloodsucker ghk, found get variable: %2",_type,_obj getVariable ["spawn_ns_bloodsucker",false]]; 
			};
		};
