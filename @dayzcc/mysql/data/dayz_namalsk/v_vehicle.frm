TYPE=VIEW
query=select `iv`.`id` AS `instance_vehicle_id`,`v`.`id` AS `vehicle_id`,`iv`.`instance_id` AS `instance_id`,`i`.`world_id` AS `world_id`,`v`.`class_name` AS `class_name`,`iv`.`worldspace` AS `worldspace`,`iv`.`inventory` AS `inventory`,`iv`.`parts` AS `parts`,`iv`.`damage` AS `damage`,`iv`.`fuel` AS `fuel` from (((`dayz_namalsk`.`instance_vehicle` `iv` join `dayz_namalsk`.`world_vehicle` `wv` on((`iv`.`world_vehicle_id` = `wv`.`id`))) join `dayz_namalsk`.`vehicle` `v` on((`wv`.`vehicle_id` = `v`.`id`))) join `dayz_namalsk`.`instance` `i` on((`iv`.`instance_id` = `i`.`id`)))
md5=93cb558fbcd241909d5e54289994727b
updatable=1
algorithm=0
definer_user=root
