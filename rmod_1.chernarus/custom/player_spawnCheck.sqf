...
if (_nearbyCount < 1) exitwith 
{
	...
	//diag_log("GHK PLAYER_SPAWNCHECK FOR WILD ZOMBIES");
		[_position] call wild_spawnZombies;
	};
	//GHK spawn in hordes for fog areas in the wilderness:
	//diag_log("GHK PLAYER_SPAWNCHECK FOR HORDE");
	
	//fog/bloodsucker zones only, check also if player is in halo jump mode for z height
	//if((!_inVehicle)&&(!dayz_won_the_game)&&(player distance [13932.31, 13369.947, 0.31807569] <= 500||player distance [8326.5879,13294.87,0.0] <= 750||player distance [3695.9475, 6004.2476, 0.13762426] <= 500|| dayz_BSZonePos <=2800)) then {
	
	if((!_inVehicle)&&(!dayz_won_the_game)) then {
	
				[_position] call zombie_generate_horde;
			
	}; 
	
};
//diag_log("GHK nearbycount building count >0");
