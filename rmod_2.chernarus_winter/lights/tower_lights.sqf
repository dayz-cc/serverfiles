...
_rng = 20; //Distance from survivor to detect towers.
...
//End Edit Values
fnc_axeTl = compile preprocessFileLineNumbers "lights\fnc_light_towers.sqf";

    if(daytime<_fnHr||daytime>_stHr)then{
	_nrPl = [8852.2676, 2866.0261, 0];
    _nrTowers = nearestObjects [_nrPl, [_twrCl], _rng];
        {
        _doLit=true;
        [_lCol,_lbrt,_lamb,[_x],[_doLit]] call fnc_axeTl;
        }forEach _nrTowers;
	_nrPl = [1965.6981, 14562.054, 0];
    _nrTowers = nearestObjects [_nrPl, [_twrCl], _rng];
        {
        _doLit=true;
        [_lCol,_lbrt,_lamb,[_x],[_doLit]] call fnc_axeTl;
        }forEach _nrTowers;	
	_nrPl = [10901.419,13612.396,0];
	_nrTowers = nearestObjects [_nrPl, [_twrCl], _rng];
        {
        _doLit=true;
        [_lCol,_lbrt,_lamb,[_x],[_doLit]] call fnc_axeTl;
