...
if (typename _this == typename []) then {
	...
	if (count _this == 2) exitwith {
		...
		_unit setVariable["ghkcanteleport",true];
		_unit setpos [position _unit select 0,position _unit select 1,_alt];
		_unit setvariable ["bis_fnc_halo_now",true];
		_radioBroadcast = ["radio1B","radio2B","radio5B"] call BIS_fnc_selectRandom;
		//cutText [format ["Welcome to UAC DayZ\n\n%1, You are humanity's last hope...", name player], "BLACK FADED"];
		cutText [format ["Welcome to UAC DayZ v1.6, %1\n", name player], "BLACK FADED"];
		sleep 5;
		cutText ["It's one year after the worldwide cataclysm...", "BLACK FADED"];
		sleep 5;
		cutText ["The pilot and everyone on board were attacked. You managed to escape with a parachute. You are the sole survivor...", "BLACK FADED"];
		sleep 5;
		if(dz_play_intro_radio) then {
			playSound _radioBroadcast;	
		};
		40 fadeSound 0.3;
		cutText ["You are one of humanity's last hope...", "BLACK FADED"];
		sleep 5;
		cutText ["Good Luck!", "BLACK FADED"];
		...
		sleep 5;
		titleCut ["", "BLACK IN", 5];
		//_radioBroadcast = ["radio1B","radio2B","radio3B","radio4B","radio5B"] call BIS_fnc_selectRandom;
		//playSound _radioBroadcast;
		player spawn p2_newspawn; //ghk
		_unit spawn bis_fnc_halo;
	};
	//-------------
	showCommandingMenu ""; //ghk
