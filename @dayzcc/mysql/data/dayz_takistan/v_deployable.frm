TYPE=VIEW
query=select `id`.`id` AS `instance_deployable_id`,`d`.`id` AS `vehicle_id`,`d`.`class_name` AS `class_name`,`s`.`id` AS `owner_id`,`p`.`name` AS `owner_name`,`p`.`unique_id` AS `owner_unique_id`,`s`.`is_dead` AS `is_owner_dead`,`id`.`worldspace` AS `worldspace`,`id`.`inventory` AS `inventory` from (((`dayz_takistan`.`instance_deployable` `id` join `dayz_takistan`.`deployable` `d` on((`id`.`deployable_id` = `d`.`id`))) join `dayz_takistan`.`survivor` `s` on((`id`.`owner_id` = `s`.`id`))) join `dayz_takistan`.`profile` `p` on((`s`.`unique_id` = `p`.`unique_id`)))
md5=2cc56138e6fd3b45c459bbd2f953c315
updatable=1
algorithm=0
definer_user=root
