TYPE=VIEW
query=select `iv`.`id` AS `instance_vehicle_id`,`v`.`id` AS `vehicle_id`,`iv`.`instance_id` AS `instance_id`,`i`.`world_id` AS `world_id`,`v`.`class_name` AS `class_name`,`iv`.`worldspace` AS `worldspace`,`iv`.`inventory` AS `inventory`,`iv`.`parts` AS `parts`,`iv`.`damage` AS `damage`,`iv`.`fuel` AS `fuel` from (((`dayz_celle`.`instance_vehicle` `iv` join `dayz_celle`.`world_vehicle` `wv` on((`iv`.`world_vehicle_id` = `wv`.`id`))) join `dayz_celle`.`vehicle` `v` on((`wv`.`vehicle_id` = `v`.`id`))) join `dayz_celle`.`instance` `i` on((`iv`.`instance_id` = `i`.`id`)))
md5=312a7e30c471010e7710ddfd305a7121
updatable=1
algorithm=0
definer_user=root
