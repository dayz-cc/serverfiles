TYPE=VIEW
query=select `id`.`id` AS `instance_deployable_id`,`d`.`id` AS `vehicle_id`,`d`.`class_name` AS `class_name`,`s`.`id` AS `owner_id`,`p`.`name` AS `owner_name`,`p`.`unique_id` AS `owner_unique_id`,`s`.`is_dead` AS `is_owner_dead`,`id`.`worldspace` AS `worldspace`,`id`.`inventory` AS `inventory` from (((`dayz_utes`.`instance_deployable` `id` join `dayz_utes`.`deployable` `d` on((`id`.`deployable_id` = `d`.`id`))) join `dayz_utes`.`survivor` `s` on((`id`.`owner_id` = `s`.`id`))) join `dayz_utes`.`profile` `p` on((`s`.`unique_id` = `p`.`unique_id`)))
md5=bd9dda0c35ffce4718fd8f1004278cfb
updatable=1
algorithm=0
definer_user=root
