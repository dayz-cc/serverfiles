...
_rainIntervalRainProbability = 1; //GHK changes these values, the rain value has to be > 0 for snow to work!
...
[_snowIntervalSnowProbability, _debug] spawn {
	...
	//diag_log("dynamic weather1 _snow:");diag_log(_snow);
	//SetRain to zero, should be no rain in winter
    if (!isDedicated) then {0 setRain 0;ghk_weather_thread_cnt = ghk_weather_thread_cnt +1;[0.1, _snow] spawn dzn_fnc_snowfall;};
    sleep 0.1;
    ...
    while {true} do {
		...
		//diag_log("dynamic weather _snow:");diag_log(_snow);
		//SetRain to zero, should be no rain in winter
        if (!isDedicated) then {0 setRain 0; ghk_weather_thread_cnt = ghk_weather_thread_cnt +1;[3, _snow] spawn dzn_fnc_snowfall;};
        
        sleep 3;
		
		//if (!isDedicated) then {diag_log("ghk_weather_thread_cnt:");diag_log(ghk_weather_thread_cnt); };
    };
};