TYPE=VIEW
query=select `iv`.`id` AS `instance_vehicle_id`,`v`.`id` AS `vehicle_id`,`iv`.`instance_id` AS `instance_id`,`i`.`world_id` AS `world_id`,`v`.`class_name` AS `class_name`,`iv`.`worldspace` AS `worldspace`,`iv`.`inventory` AS `inventory`,`iv`.`parts` AS `parts`,`iv`.`damage` AS `damage`,`iv`.`fuel` AS `fuel` from (((`dayz_fallujah`.`instance_vehicle` `iv` join `dayz_fallujah`.`world_vehicle` `wv` on((`iv`.`world_vehicle_id` = `wv`.`id`))) join `dayz_fallujah`.`vehicle` `v` on((`wv`.`vehicle_id` = `v`.`id`))) join `dayz_fallujah`.`instance` `i` on((`iv`.`instance_id` = `i`.`id`)))
md5=85acdf45ab3a125d975b7f6fc11d239d
updatable=1
algorithm=0
definer_user=root
