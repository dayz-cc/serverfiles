...
if (isset($_GET['delete'])) {
	...
	if (isset($_POST['backpack'])) {
		mysql_query("UPDATE `survivor` SET `backpack` = '".(mysql_real_escape_string($_POST["backpack"]))."' WHERE `unique_id` = '{$_GET['uid']}' AND `id` = '{$_GET["id"]}'");
	}

	if (isset($_POST['model'])) {
		if ($_POST['model'] != '' && $_POST['model'] != '""') { mysql_query("UPDATE `survivor` SET `model` = '".(str_replace('"', '', $_POST["model"]))."' WHERE `unique_id` = '{$_GET['uid']}' AND `id` = '{$_GET["id"]}'"); }
	}
}

$res = mysql_query("SELECT profile.*, survivor.* FROM `profile`, `survivor` WHERE profile.unique_id = survivor.unique_id AND survivor.unique_id = '".$_GET["uid"]."' AND survivor.id = '".$_GET["id"]."' LIMIT 1") or die(mysql_error());
...
<div id="page-heading">
	...
	<h1><?php echo $name." - ".$row['unique_id']." - Last save: ".$row['last_updated']." - Position: ".$row['worldspace']; ?></h1>
</div>
					...
					<div id="gear_player">
						...
						<div class="gear_info">
							...
							<div class="statstext" style="width: 180px; margin-left: 205px; margin-top: -115px">
								<?php echo 'Zombie kills:&nbsp;'.$row['zombie_kills']; ?>
							</div>
							<div class="statstext" style="width: 180px; margin-left: 205px; margin-top: -95px">
								<?php echo 'Zombie headshots:&nbsp;'.$row['headshots']; ?>
							</div>
							<div class="statstext" style="width: 180px; margin-left: 205px; margin-top: -75px">
								<?php echo 'Survivors killed:&nbsp;'.$row['survivor_kills']; ?>
							</div>
							<div class="statstext" style="width: 180px; margin-left: 205px; margin-top: -55px">
								<?php echo 'Bandits killed:&nbsp;'.$row['bandit_kills']; ?>
							</div>
							<div class="statstext" style="width: 180px; margin-left: 205px; margin-top: -35px">
								<a href="javascript:if (confirm('Are you sure you want to delete this player?')) { window.location='index.php?view=info&show=1&uid=<? echo $row['unique_id']; ?>&id=<? echo $row['id']; ?>&delete'; }">Delete</a>
							</div>
						...
						<div class="gear_inventory">
							...
							<div class="inventoryheader">
								<?php echo "Start Time: ".$row['start_time']; ?>
							</div>
						</div>
						<!-- Backpack -->
