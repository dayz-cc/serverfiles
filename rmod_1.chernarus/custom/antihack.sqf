...
while {alive player} do
{
	...
	//GHK Remove this for now
	//if (!(player getVariable["ghkcanteleport",false])&&(_speed > _topSpeed) && (alive player) && ((driver (vehicle player) == player) or (isNull (driver (vehicle player)))) && (_debug distance _lastpos > 3000) && !((vehicle player == player) && (_curheight < _lastheight) && ((_curheight - _terrainHeight) > 1))) then {
