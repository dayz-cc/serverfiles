...
//GHK since _player == player:
//dont allow vehicles to teleport.
 if ( vehicle player != player ) exitWith {};

player setVariable["ghkcanteleport",true]; //override player_spawn_2 checks. --could cause some problems with re-spawning?
...
switch (_telelocation) do
	...
	case 18:
		...
		//_markerveh="Can_Small" createVehicleLocal [7901.9189,13685.755];(vehicle player) attachto [_markerveh,[0.0,0.0,0.2]];sleep 5; detach (vehicle player);sleep 5; detach player;
		//_dummyveh = "PBX" createVehicleLocal [7901.9189, 13685.755];player moveInDriver _dummyveh;deleteVehicle _dummyveh;
		};
	case 19:
		{
	...
	if (count _myPos > 0) then {player setVariable["lastPos",(getPosATL player),true];player setVariable["lastPos",[]];};
