...
if (_hasdog == 0) then {
   ...
   //player removeWeapon 'CDF_dogtags';
    ...
    if(!("CDF_dogtags" in Weapons player)) then { //player should always have a dogtag to get this far.
	...
	//diag_log ("ghk got tame: characterid" + str (player getVariable ["characterID", "0"]) );
    _dog setVariable ["CharacterID",dayz_characterID, true];
	
	//diag_log ("ghk got tame: characterid" + str (_dog getVariable ["characterID", "0"]) );
	...
	player setVariable ["hadDog", 1]; //ghk used for when player's dog died, but then went to change costum
...
_nul = [objNull, _dog, rSAY, "bark"] call RE; 
	...
	//diag_log ("ghk got tame2: characterid" + str (_dog getVariable ["characterID", "0"]) );
	...
	//[nil, player, "loc", rHINT, _txt, "PLAIN DOWN"] call RE;
		...
		if (_commd == "feed") then
		{
			...
			if (!_hasRMeat) then {
				...
				systemChat "You have no raw meat to feed your dog.";
				_nul = [objNull, _dog, rSAY, "growl"] call RE;
			} else {
				player removeMagazine _theMeat;
				cutText [format["Your dog ate some meat."], "PLAIN DOWN"];
				systemChat "Your dog ate some meat.";
				_FutureFeedTime = floor(time) + 3600 + floor(random 7200); //3600 = 1 hour
				_didPreFeed = false;
				_nul = [objNull, _dog, rSAY, "bark"] call RE; 
