...
//THIS IS USED IN DZMS, DZMSHOTSPOTS AND WAI to only spawn 1 unit per spawn position for low player count, set to 0 for multiplayer
dz_singleplayer_missions=1;
//Used in DZMS, DZMSHOTSPOTS AND WAI to not spawn vehicles or heli para backup, and to force all AI spawned to be at lowest skill level, great for single player
...
dz_horde = true; 				//Spawn chance for a horde to spawn(if dayz zombie limit allows) //only in the wild (200m away from any building)
...
dz_max_bloodsuckers = 1; //maximum amount of bloodsuckers allowed within 510m of a player, set to 0 to disable bloodsuckers
